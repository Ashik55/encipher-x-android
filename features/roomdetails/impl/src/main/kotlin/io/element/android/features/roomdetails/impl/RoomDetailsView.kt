...
import io.element.android.libraries.designsystem.atomic.molecules.CustomMatrixBadgeRowMolecule
...
import io.element.android.libraries.designsystem.components.button.CustomMainActionButton
...
private fun BadgeList(
    ...
    Box(modifier = modifier) {
        if (roomBadge.isNotEmpty()) {
            CustomMatrixBadgeRowMolecule(
                data = roomBadge.map {
                    it.toCustomMatrixBadgeData()
                }.toImmutableList(),
            )
        }
    }
...
@Composable
private fun RoomBadge.toCustomMatrixBadgeData(): CustomMatrixBadgeAtom.CustomMatrixBadgeData {
    return when (this) {
        RoomBadge.ENCRYPTED -> {
...
private fun NotificationItem(
    ...
    ListItem(
        headlineContent = { Text(text = stringResource(R.string.screen_room_details_notification_title), color = Color(0xFF11181C)) },
...
private fun PinnedMessagesItem(
    ...
    ListItem(
        ...
        if (pinnedMessagesCount == null) {
            ...
            ListItemContent.Custom{
                Row {
                    Text(pinnedMessagesCount.toString())
    ...
    PreferenceCategory(title = stringResource(R.string.screen_room_details_security_title)) {
        ListItem(
            headlineContent = { Text(
                text = stringResource(R.string.screen_room_details_encryption_enabled_title),
                color = Color(0xFF0A8741)
            )  },
            supportingContent = { Text(stringResource(R.string.screen_room_details_encryption_enabled_subtitle)) },
            leadingContent = ListItemContent.Custom {
                Icon(
                    painter = painterResource(id = CommonDrawables.ic_encryption_enabled),
                    contentDescription = null,
